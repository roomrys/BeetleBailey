...
spi_transaction_t set_spi_trans(int cmd, int addr)
{
    // Set the transaction configuration structure
    spi_transaction_t spi_trans={
        .flags=SPI_TRANS_USE_TXDATA,                 ///< Bitwise OR of SPI_TRANS_* flags
        .cmd=cmd,                   /**< Command data, of which the length is set in the ``command_bits`` of spi_device_interface_config_t.
                                    *
                                    ...
                                    */
        .addr=addr,                  /**< Address data, of which the length is set in the ``address_bits`` of spi_device_interface_config_t.
                                    *
                                    ...
                                    */
        .length=4,                  ///< Total data length, in bits
...
spi_transaction_t MAX7219_display_test()
{
    // Set the transaction configuration structure
    int cmd = 0x0F;
    int addr = 0x0c;
    spi_transaction_t norm_op = set_spi_trans(cmd, addr);
    return norm_op;
}
...
void app_main()
{
    ...
    // Initialize the bus configuration structure
    ESP_LOGI(tag, "... Initializing bus.");
    ESP_ERROR_CHECK(spi_bus_initialize(HSPI_HOST, &buscfg, 1));

    // Set the device configuration structure
    ...
    while(1){
        ...
        // Set the transaction configuration structure
        spi_transaction_t spi_trans = MAX7219_display_test();
